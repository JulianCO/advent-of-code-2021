...
winningBoard :: [Int] -> [BingoBoard] -> (BingoBoard, Int)
winningBoard called boards = 
    let boards' = map (callNumber (head called)) boards in
        case filter hasWon boards' of
            [] -> winningBoard (tail called) boards'
            (b:_) -> (b, head called)
...
main = do
    ...
    let (winnerBoard, lastCalled) = winningBoard numbersCalled allBoards
    --print numbersCalled
    --print allBoards
    --print winnerBoard
    print (bingoScore winnerBoard lastCalled)
