...
func solution(graph *map[string][]string) int {
	...
	return countPaths("start", graph, &smallCave, &visited, false)
}

func countPaths(start string, graph *map[string][]string, smallCave *map[string]bool, visited *map[string]bool, haveTwice bool) int {
	if (*smallCave)[start] && (*visited)[start] && haveTwice {
		return 0
	}
	if start == "start" && (*visited)[start] {
		return 0
	}
	if start == "end" {
		return 1
	...
	if (*smallCave)[start] && (*visited)[start] {
		...
		for _, neighbor := range (*graph)[start] {
			total += countPaths(neighbor, graph, smallCave, visited, true)
		}
		return total
	} else {
		(*visited)[start] = true
		total := 0
		for _, neighbor := range (*graph)[start] {
			total += countPaths(neighbor, graph, smallCave, visited, haveTwice)
		}
		(*visited)[start] = false
		return total
	}
